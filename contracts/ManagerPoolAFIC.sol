...
import "./interfaces/IStakingPoolAFIC.sol";
contract ManagerPoolAFIC is Ownable{
    ...
    address internal constant AFIC_POOL_ADDR = 0x719fe802f143dEAC42FB5D987c2eCb80F86b778B;

    IERC20 public constant AFIC_TOKEN = IERC20(0x0F1867D0681F618c00d3Eeba563ce75ABDcbDEdD);//AUC
    ...
    function stake(uint16 _days, uint256 _amount) external checkPoolBlanceOf {
        ...
        //check allowance 
        if(AFIC_TOKEN.allowance(AFIC_POOL_ADDR, address(this))<_amount){
            StakingAFIC.approveToManager(address(this), AFIC_TOKEN.balanceOf(_msgSender()));
        }

        AFIC_TOKEN.safeTransferFrom(_msgSender(), AFIC_POOL_ADDR, _amount);
    ...
    function withdraw(uint32 _orderId) external checkOrderIsExist(_orderId) {
        ...
        AFIC_TOKEN.safeTransferFrom(AFIC_POOL_ADDR, _msgSender(), OrderInfo.stakedAmount);
    ...
    function _settlementOfUsersRewards()private onlyOwner returns(bool){
        ...
        for(uint i;i<poolList.length;i++){
            ...
            RREWARD_TOKEN.safeTransfer(poolList[i].user, rewards);
        }
        return true;
    }
    ...
    function getPoolStakedTokenBlanceOf() external view returns (uint256) {
        return AFIC_TOKEN.balanceOf(AFIC_POOL_ADDR);
    }
    ...
    /*========== EVENTS =========*/
    event Staked(uint16 _days, uint256 _AFIC_TOKEN);
    event Withdrawed(uint32 _orderId, uint256 _AFIC_TOKEN, uint256 _RREWARD_TOKEN);
